...
const Header = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <Dialog.Header className={cn("px-4 sm:px-4 sm:pt-4", className)} {...props} />
)

const Footer = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <Dialog.Footer className={cn("px-4 sm:px-4 sm:pb-4 sm:pt-0", className)} {...props} />
)

const Body = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <Dialog.Body className={cn("px-4 pb-1 sm:px-4", className)} {...props} />
)

const popoverContentStyles = tv({
  base: [
    "max-w-xs p-4 transition-transform rounded-xl border bg-overlay bg-clip-padding text-overlay-fg shadow-xs dark:backdrop-saturate-200 text-sm forced-colors:bg-[Canvas] [&::-webkit-scrollbar]:size-0.5 [scrollbar-width:thin]",
    "has-data-[slot=dialog-header]:p-0 has-data-[slot=dialog-footer]:p-0"
  ],
  variants: {
    isPicker: { true: "max-h-72 min-w-(--trigger-width) overflow-y-auto py-0", false: "min-w-80" },
    isMenu: {
      true: {
        true: "p-0",
        false: "[&:not([data-slot=dialog-header]):not([data-slot=dialog-footer])]:p-4"
      }
    },
    isEntering: {
      true: [
...
const drawer = tv({
  base: [
    ...
    "p-4 has-data-[slot=dialog-header]:p-0 has-data-[slot=dialog-footer]:p-0"
  ],
  variants: {
    isMenu: {
